...
const MapContainer: React.FC<MapContainerProps> = ({
  ...
  const { mapCenter, mapZoom, userLocation } = useAppSelector(
    (state) => state.ui,
  );
  ...
  // Use snappedCoordinates (road-following) when available, fall back to raw geometry
  const routeFeatures = useMemo((): RouteFeature[] => {
    ...
    return routes.map((route) => ({
      ...
      geometry: {
        type: "LineString",
        coordinates:
          route.snappedCoordinates || route.geometry?.coordinates || [],
      },
      properties: {
        id: route.id,
  ...
  // Route style function - memoized to update when selectedRoute changes
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Leaflet Map | Center: {validCenter[0].toFixed(4)},{" "}
          {validCenter[1].toFixed(4)}
        </Typography>
      </Box>
      ...
      >
        ...
        {routeFeatures.length > 0 && (
          <GeoJSON
            key={`routes-${selectedRoute?.id || "none"}`}
            data={
              {
        ...
        {busFeatures.map((bus) => (
          ...
          >
            <Popup>
              <div>
                <strong>Bus {bus.properties.busNumber}</strong>
                <br />
                Route: {bus.properties.routeName}
                <br />
                Status: {bus.properties.occupancyStatus}
                <br />
                Passengers: {bus.properties.currentPassengers}/
                {bus.properties.capacity}
              </div>
            </Popup>
          </Marker>
        ))}
