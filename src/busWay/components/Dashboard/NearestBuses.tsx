...
const NearestBuses: React.FC = () => {
  ...
  const findNearestBuses = useCallback(async () => {
    ...
    try {
      await dispatch(fetchNearestBuses({
  ...
  const handleRadiusChange = useCallback((_: Event, newValue: number | number[]) => {
      ...
      >
        <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
            ...
            <Box>
              <Typography variant="h6" fontWeight={600}>Nearest Buses</Typography>
              <Typography variant="caption" color="text.secondary">
                {busesWithDistance.length} buses within {formatDistance(searchRadius)}
              </Typography>
            </Box>
          </Box>
